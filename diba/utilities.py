...
def get_number_from_name(name: AstrologicalPoint) -> int:
    ...
    Raises:
        DibaException: If the name is not recognized
    """
    try:
        return _POINT_NUMBER_MAP[str(name)]
    except KeyError as exc:
        raise DibaException(f"Error in getting number from name! Name: {name}") from exc


def get_diba_point_from_degree(
    ...
    """
    Create a DibaPointModel from a degree position.
    ...
    Returns:
        A DibaPointModel with calculated zodiac sign, position, and properties

    Raises:
        DibaException: If the degree is >= 360 after normalization
    """
    ...
    if degree >= 360:
        raise DibaException(f"Error in calculating positions! Degrees: {degree}")

    sign_index = int(degree // 30)
    sign_degree = degree % 30
    zodiac_sign = _ZODIAC_SIGNS[sign_index]

    return DibaPointModel(
        name=name,
...
def is_point_between(
    ...
    Raises:
        DibaException: If the arc exceeds 180°
    """
    ...
    if span > 180:
        raise DibaException(f"The angle between start and end point is not allowed to exceed 180°, yet is: {span}")
    if target == start:
        return True
    if target == end:
        return False
...
def get_houses_list(
    ...
    Returns:
        List of DibaPointModel objects representing the houses
    """
...
def get_available_astrological_points_list(subject: AstrologicalSubjectModel) -> list[DibaPointModel]:
    """
    ...
    Returns:
        List of DibaPointModel objects for all active points
    """
...
def get_moon_emoji_from_phase_int(phase: int) -> LunarPhaseEmoji:
    ...
    Raises:
        DibaException: If phase is outside valid range
    """
    ...
    if phase == 1:
        ...
        raise DibaException(f"Error in moon emoji calculation! Phase: {phase}")
...
def get_moon_phase_name_from_phase_int(phase: int) -> LunarPhaseName:
    ...
    if phase == 1:
        ...
        raise DibaException(f"Error in moon name calculation! Phase: {phase}")
