...
This module contains all Pydantic models used throughout the Diba library
for representing astrological data structures.

Model Hierarchy:
    SubscriptableBaseModel
    ├── LunarPhaseModel - Moon phase information
    ├── DibaPointModel - Celestial points (planets, houses, etc.)
...
class DibaPointModel(SubscriptableBaseModel):
    """
...
class VedicSettingsModel(SubscriptableBaseModel):
    """
    Vedic settings container.
    """
...
class VedicCorePointModel(SubscriptableBaseModel):
    """
    Minimal sidereal core point.
    """

    abs_pos_sidereal: float


class VedicCoreModel(SubscriptableBaseModel):
    """
    Container for sidereal core points.
    """

    jd_utc: float
    objects: dict[str, VedicCorePointModel]


class VedicRasiPointModel(SubscriptableBaseModel):
    """
    Minimal Rasi (D1) point data.
    """
...
class VedicRasiHousesModel(SubscriptableBaseModel):
    """
    House mapping for Rasi (D1).
    """

    model: Literal["whole_sign"]
    houses: list[dict[str, object]]


class VedicRasiD1Model(SubscriptableBaseModel):
    """
    Rasi (D1) container.
    """

    points: dict[str, VedicRasiPointModel]
    houses: VedicRasiHousesModel


class VedicVargaPointModel(SubscriptableBaseModel):
    """
    Minimal Varga point data.
    """
...
class VedicVargaChartModel(SubscriptableBaseModel):
    """
    Single Varga chart container.
    """

    factor: int
    method: int
    method_label: Optional[str] = None
    lagna: Optional[VedicVargaPointModel] = None
    points: dict[str, VedicVargaPointModel]


class VedicVargasSettingsModel(SubscriptableBaseModel):
    """
    Varga settings container.
    """

    requested: list[str]
    default_method_by_chart: dict[str, int]
    methods: dict[str, int]
    profile: Optional[str] = None


class VedicVargasModel(SubscriptableBaseModel):
    """
    Vargas payload container.
    """

    settings: VedicVargasSettingsModel
    charts: dict[str, VedicVargaChartModel]


class VedicModel(SubscriptableBaseModel):
    """
    Root Vedic payload for additive JSON output.
    """

    settings: VedicSettingsModel
    ayanamsa_deg: float
    core: VedicCoreModel
    rasi_d1: VedicRasiD1Model
    vargas: Optional[VedicVargasModel] = None
...
class AstrologicalBaseModel(SubscriptableBaseModel):
    ...
    sun: Optional[DibaPointModel] = None
    moon: Optional[DibaPointModel] = None
    mercury: Optional[DibaPointModel] = None
    venus: Optional[DibaPointModel] = None
    mars: Optional[DibaPointModel] = None
    jupiter: Optional[DibaPointModel] = None
    saturn: Optional[DibaPointModel] = None
    uranus: Optional[DibaPointModel] = None
    neptune: Optional[DibaPointModel] = None
    pluto: Optional[DibaPointModel] = None

    # Common axes
    ascendant: Optional[DibaPointModel] = None
    descendant: Optional[DibaPointModel] = None
    medium_coeli: Optional[DibaPointModel] = None
    imum_coeli: Optional[DibaPointModel] = None

    # Additive Vedic payload (optional)
    vedic: Optional[VedicModel] = None

    # Common optional planets
    chiron: Optional[DibaPointModel] = None
    earth: Optional[DibaPointModel] = None
    pholus: Optional[DibaPointModel] = None

    # Lilith Points
    mean_lilith: Optional[DibaPointModel] = None
    true_lilith: Optional[DibaPointModel] = None

    # Asteroids
    ceres: Optional[DibaPointModel] = None
    pallas: Optional[DibaPointModel] = None
    juno: Optional[DibaPointModel] = None
    vesta: Optional[DibaPointModel] = None

    # Trans-Neptunian Objects
    eris: Optional[DibaPointModel] = None
    sedna: Optional[DibaPointModel] = None
    haumea: Optional[DibaPointModel] = None
    makemake: Optional[DibaPointModel] = None
    ixion: Optional[DibaPointModel] = None
    orcus: Optional[DibaPointModel] = None
    quaoar: Optional[DibaPointModel] = None

    # Fixed Stars
    regulus: Optional[DibaPointModel] = None
    spica: Optional[DibaPointModel] = None

    # Arabic Parts
    pars_fortunae: Optional[DibaPointModel] = None
    pars_spiritus: Optional[DibaPointModel] = None
    pars_amoris: Optional[DibaPointModel] = None
    pars_fidei: Optional[DibaPointModel] = None

    # Special Points
    vertex: Optional[DibaPointModel] = None
    anti_vertex: Optional[DibaPointModel] = None

    # Common houses
    first_house: DibaPointModel
    second_house: DibaPointModel
    third_house: DibaPointModel
    fourth_house: DibaPointModel
    fifth_house: DibaPointModel
    sixth_house: DibaPointModel
    seventh_house: DibaPointModel
    eighth_house: DibaPointModel
    ninth_house: DibaPointModel
    tenth_house: DibaPointModel
    eleventh_house: DibaPointModel
    twelfth_house: DibaPointModel

    # Lunar Nodes
    mean_north_lunar_node: Optional[DibaPointModel] = None
    true_north_lunar_node: Optional[DibaPointModel] = None
    mean_south_lunar_node: Optional[DibaPointModel] = None
    true_south_lunar_node: Optional[DibaPointModel] = None
...
class AstrologicalSubjectModel(AstrologicalBaseModel):
    ...
    julian_day: float
    julian_day_local: Optional[float] = None
    day_of_week: str
...
class EphemerisDictModel(SubscriptableBaseModel):
    ...
    date: str
    planets: List[DibaPointModel]
    houses: List[DibaPointModel]
