...
class VedicRegistryError(ValueError):
    ...
    Examples:
        ...
        ...
        kerykeion.vedic.registry.VedicRegistryError: Invalid ayanamsa
    """
...
def resolve_ayanamsa(mode: str, *, user_value: Optional[float] = None) -> AyanamsaSpec:
    ...
    if spec.status != "implemented":
        raise VedicRegistryError(
            f"Ayanamsa '{mode}' is recognized but not implemented yet in Kerykeion runtime."
        )
...
def resolve_house_system(sys_id: str) -> HouseSystemSpec:
    ...
    if spec.status != "implemented":
        raise VedicRegistryError(
            f"House system '{sys_id}' is recognized but not implemented yet in Kerykeion runtime."
        )
...
def resolve_node_mode(node_mode: str) -> NodeModeSpec:
    ...
    if spec.status != "implemented":
        raise VedicRegistryError(
            f"Node mode '{node_mode}' is recognized but not implemented yet in Kerykeion runtime."
        )
    return spec
